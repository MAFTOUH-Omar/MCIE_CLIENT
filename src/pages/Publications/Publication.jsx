...
const Publication = () => {
    ...
    const fetchData = async ( order = 'recent') => {
        try {
            setLoading(true)
            const response = await axios.get(import.meta.env.VITE_API_PATH + `/publication?sort=${order}`);
            if ( response.data ) {
  ...
  return (
    <div className='flex md:mx-20 md:mt-28 flex-col'>
        ...
        <div className='flex justify-end items-end w-full flex-col'>
            <h1 className='text-xl font-bold title'>صفحة المنشورات</h1>
            <p className='text-slate-500 dark:text-slate-400'>تستعرض أحدث المنشورات في مختلف المجالات</p>
        </div>
